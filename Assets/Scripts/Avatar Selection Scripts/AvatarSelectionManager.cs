...
public class AvatarSelectionManager : MonoBehaviour
{
    ...
    private void LoadAvatarModelAt(int avatarIndex)
    {
        ...
        Transform bodyModel = loadableAvatarModels[avatarIndex].GetComponent<AvatarHolder>().BodyTransform;
        Transform neckModel = bodyModel.Find("Hips/Spine/Neck");
        if (neckModel != null) {
            avatarInputConverter.AvatarBody = neckModel;
        }
        else {
            Debug.Log("Neck NOT FOUND!");
            avatarInputConverter.AvatarBody = bodyModel;
        }

        Transform headModel = loadableAvatarModels[avatarIndex].GetComponent<AvatarHolder>().HeadTransform;
        Transform readyMeHead = headModel.Find("Hips/Spine/Neck/Head");
        if (readyMeHead != null) {
            Debug.Log("readyMeHead FOUND!");
            Transform actualHead = readyMeHead;
            avatarInputConverter.AvatarHead = actualHead;

        }
        else {
            Debug.Log("readyMeHead NOT FOUND!");
            avatarInputConverter.AvatarHead =headModel;
        }
