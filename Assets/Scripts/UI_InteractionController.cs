...
public class UI_InteractionController : MonoBehaviour
{
    ...
    private void ActivateUIMode(InputAction.CallbackContext obj)
    {
        if (!isUICanvasActive)
        {
            ...
            //Deactivating Base Controller by disabling its XR Direct/Ray Interactor
            if (BaseController.GetComponent<XRRayInteractor>())
                BaseController.GetComponent<XRRayInteractor>().enabled = false;
            else if (BaseController.GetComponent<XRDirectInteractor>())
        ...
        else
        {
            ...
            //Activating Base Controller by disabling its XR Direct/Ray Interactor
            if (BaseController.GetComponent<XRRayInteractor>())
